...
tenantDb.detail_properties.insertMany([
    { dataset_name: DATASET_NAME, name: "description",    type: "json", path: "$",   order: 1 }
]);
tenantDb.detail_properties.deleteMany({ dataset_name: `${DATASET_NAME}-plaatsnaam` });
tenantDb.detail_properties.insertMany([
    { dataset_name: `${DATASET_NAME}-plaatsnaam`, name: "description", type: "json", path: "$", order: 1 }
]);
tenantDb.detail_properties.deleteMany({ dataset_name: `${DATASET_NAME}-personen` });
tenantDb.detail_properties.insertMany([
    { dataset_name: `${DATASET_NAME}-personen`, name: "description", type: "json", path: "$", order: 1 }
]);
tenantDb.detail_properties.deleteMany({ dataset_name: `${DATASET_NAME}-uitgever_drukker` });
tenantDb.detail_properties.insertMany([
    { dataset_name: `${DATASET_NAME}-uitgever_drukker`, name: "description", type: "json", path: "$", order: 1 }
]);
